...
class Client:
    ...
    def get_simulation_id(self):
        ...
        if len(response) > 1:
            raise ValueError(f"expecting only one simulation with name={self.sim_name} found {response}")
        elif len(response) ==0:
    ...
    def auth(self):
        ...
        req["authNonce"] = (str(int(time.time()))+ "-"+ "".join(rand_num.choice(numconv.BASE62) for x in range(32)))
        hash_val = hashlib.sha256()
        hash_val.update(":".join([req["authNonce"],req["simulationType"],req["simulationId"],"bluesky",]).encode())
    ...
    def _post_json(self, url, payload):
        ...
        response = requests.post("{}/{}".format("http://localhost:8000", url),json=payload,cookies=self.cookies,)
    ...
    def post(self, path, data):
        ...
        response = self.session.post(f"{self.host}/{path}",json=data,)
    ...
    def run_simulation(self, data, max_calls=1000):
        ...
        for i in range(max_calls):
            ...
            if state == "completed" or state == "error":
                break
            assert ("nextRequestSeconds" in response), 'missing "nextRequestSeconds" in response:{}'.format(response)
